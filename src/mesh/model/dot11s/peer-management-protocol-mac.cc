...
{
  NS_LOG_FUNCTION (this << interface << protocol);
  m_ifIndex = interface;
  m_protocol = protocol;
}
...
PeerManagementProtocolMac::GetParent ()
{
  return m_parent;
}
void
...
void
PeerManagementProtocolMac::UpdateFailAvg (Mac48Address peerAddress, double failAvg)
{
  m_parent->UpdateFailAvg (peerAddress, failAvg);
}
void
...
WifiMode
PeerManagementProtocolMac::GetDataTxWifiMode(Mac48Address peerAddress)
{
  return m_parent->GetDataTxWifiMode (peerAddress);
}
int64_t
